...
def get_p2wpkh_script(pubkeyhash):
    """Get the script associated with a P2WPKH."""
    return CScript([CScriptOp(OP_0), pubkeyhash])
...
def taproot_construct(pubkey, scripts=[]):
    ...
    if len(scripts) == 0:
        ...
        return (CScript([OP_1, tweaked.get_bytes()]), bytes([0 for i in range(32)]), {})
